...
foreach( $xml->channel->item as $key => $value){
	...
	echo "<h1>$value->title <br /></h1>";
	echo "$value->description<br />";
	echo "<a href='$value->link'>$value->link</a><br />";
	
	echo "$value->location<br />";
	
	echo "$value->dtstart<br />";
	echo "$value->dtend<br />";
	...
	$grab_start_time 	= $value->dtstart;
		$start_time		= strtotime($grab_start_time);
	$grab_end_time		= $value->dtend;
		$end_time		= strtotime($grab_end_time);
	...
	*/
	...
	/*
	$query = "INSERT INTO events (postFacebookID, accountFacebookID, accountName, name, description, start_time, end_time, location, street, city, state, zip, country, latitude, longitude, privacy, updated_time, image, category) VALUES ('$postFacebookID', '$accountFacebookID', '$accountName', '$name', '$description', '$start_time', '$end_time', '$location', '$street', '$city', '$state', '$zip', '$country', '$latitude', '$longitude', '$privacy', '$updated_time', '$image', 'community')";
